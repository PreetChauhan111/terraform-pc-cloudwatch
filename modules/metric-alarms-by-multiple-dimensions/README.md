...
Wrapper module for AWS CloudWatch metric alarms created across multiple dimensions.
...
```hcl
module "metric_alarms_multi_dim" {
  source = "./modules/metric-alarms-by-multiple-dimensions"

  environment         = "dev"
  region              = "ap-south-1"
  alarm_name          = "pc-dev-cpu"
  ...
  statistic           = "Average"
  dimensions = {
    app1 = { InstanceId = "i-0123456789abcdef0" }
    app2 = { InstanceId = "i-0fedcba9876543210" }
  }
}
```
...
| `alarm_name_delimiter` | Delimiter between alarm name and dimension key. Use dash or underscore for pretty alert names. | `string` | `""` |
...
| `threshold` | The value against which the specified statistic is compared. | `number` | `n/a` |
...
| `dimensions` | The dimensions for the alarm's associated metric. | `any` | `{}` |
